...
| `dev-tooling/` | `check_plugin_structure.py` (the Codex-layout structural gate). The commit-refuse hook is no longer a vendored `git-hook-wrapper.sh` scaffold here — `just bootstrap` installs the canonical structural hook from the shared `livespec-dev-tooling` package (`python -m livespec_dev_tooling.install_commit_refuse_hooks`, the SINGLE source of the hook body; pinned in `pyproject.toml`). |
