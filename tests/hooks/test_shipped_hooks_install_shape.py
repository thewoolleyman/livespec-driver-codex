...
_GUARD_CORPUS = (
    ...
    # guard. `tests/hooks/test_footgun_tmux.py` pins them against the
    # classifier in-process; these re-run them against the PACKAGED subtree
    # under a bare interpreter, which is how the guard actually runs.
    ("env -i clears TMUX_TMPDIR", "env -i tmux kill-server", "deny"),
    ("env -u wrapper", "env -u TMUX_TMPDIR tmux kill-server", "deny"),
    ("command builtin", "command tmux kill-server", "deny"),
    ("exec prefix", "exec tmux kill-server", "deny"),
    ("sudo prefix", "sudo tmux kill-server", "deny"),
    ("sudo with flag", "sudo -u ubuntu tmux kill-server", "deny"),
    ("nice prefix", "nice tmux kill-server", "deny"),
    ("nice -n argument", "nice -n 5 tmux kill-server", "deny"),
    ("timeout duration", "timeout 5 tmux kill-server", "deny"),
    ("nohup", "nohup tmux kill-server", "deny"),
    ("setsid", "setsid tmux kill-server", "deny"),
    ("stdbuf", "stdbuf -oL tmux kill-server", "deny"),
    ("ionice", "ionice -c2 -n7 tmux kill-server", "deny"),
    ("time", "time tmux kill-server", "deny"),
    ("mise exec", "mise exec -- tmux kill-server", "deny"),
    ("stacked wrappers", "sudo env -i timeout 5 tmux kill-server", "deny"),
    ("clustered default label", "tmux -Ldefault kill-server", "deny"),
    ("label after subcommand", "tmux kill-server -L default", "deny"),
    ("double slash socket", "tmux -S /tmp/tmux-1000//default kill-server", "deny"),
    ("dotdot socket", "tmux -S /tmp/tmux-1000/../tmux-1000/default kill-server", "deny"),
    ("trailing slash socket", "tmux -S /tmp/tmux-1000/default/ kill-server", "deny"),
    ("namespace sibling socket", "tmux -S /tmp/tmux-1000/other kill-server", "deny"),
    ("sh -lc login shell", "sh -lc 'tmux kill-server'", "deny"),
    ("zsh -ic interactive", "zsh -ic 'tmux kill-server'", "deny"),
    ("doubly nested shells", "bash -c \"bash -c 'tmux kill-server'\"", "deny"),
    ("xargs indirect", "echo kill-server | xargs tmux", "deny"),
    ("xargs with flags", "echo x | xargs -n 1 tmux", "deny"),
    ("command substitution", "$(echo tmux) kill-server", "deny"),
    ("semicolon chain", "echo hi; tmux kill-server", "deny"),
    ("or chain", "false || tmux kill-server", "deny"),
    ("newline chain", "echo hi\ntmux kill-server", "deny"),
    ("background operator", "tmux kill-server &", "deny"),
    ("killall with signal", "killall -9 tmux", "deny"),
    ("pkill server binary", "pkill -f /usr/bin/tmux", "deny"),
    ("unbalanced quote hazard", "tmux kill-server '", "deny"),
    ...
    ("scoped under env -i", "env -i tmux -L scratch9 kill-server", "allow"),
    ("scoped under exec", "exec tmux -L scratch9 kill-server", "allow"),
    ("scoped xargs target", "echo kill-server | xargs tmux -L scratch9", "allow"),
    ("scoped label after subcommand", "tmux kill-server -L scratch99", "allow"),
    ("new scoped session", "tmux -L scratch new -d -s probe", "allow"),
    ("grep pattern", "git log --grep='tmux kill-server'", "allow"),
    ("commit message", "git commit -m 'never run tmux kill-server'", "allow"),
    ("heredoc body", "cat <<'EOF'\ntmux kill-server\nEOF", "allow"),
    ("python string", "python3 -c \"print('tmux kill-server')\"", "allow"),
    ("unbalanced quote benign", "echo 'unterminated", "allow"),
)
